...
export class DishesService {
  ...
  async loadDishes() {
     this.webRequestService
      ...
      .subscribe((res: any) => {
        ...
        this.dishesChangedEvent.emit(dishes)
    })
  }
  ...
  getDishWithID(id: string): Observable<Dish> {
    return this.webRequestService.get(`dishes/${id}`)
  }
  ...
  addDish(dish: Dish) {
    ...
    //   dish = res.data // Overwrite the old one dish
    ...
    this.webRequestService.post('dishes', dish).subscribe(() => {
      const dishPrice = this.currencyService.calcDishReferencePrice(dish)
      if (dishPrice > this.maxDishPrice) this.maxDishPrice = +dishPrice.toFixed(2)
      if (dishPrice < this.minDishPrice) this.minDishPrice = +dishPrice.toFixed(2)
      this.loadDishes()
